...
# simulate_project.sh - Run simulation for project-specific sources.
#
# Usage:
#   ./simulate_project.sh [--verbose|-v] [--tb testbench_file.v] path/to/verilog_file.v ...
...
usage() {
    echo "Usage: $0 [--verbose|-v] [--tb testbench_file.v] path/to/verilog_file.v ..."
    exit 1
}
...
if [ -n "$TB_FILE" ]; then
    # If provided and no slash, prepend project's test directory.
    if [[ "$TB_FILE" != *"/"* ]]; then
    ...
    # Search the project's test directory for *_tb.v
